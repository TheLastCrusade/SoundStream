...
/**
 * Borrowed from Android's RandomPlayer example, with some additions to integrate it into SoundStream.
 * 
...
public class RemoteControlClientCompat {
    ...
    private AudioManager mRegisteredAudioManager;
    ...
    /**
     * Register this client.  This is a helper method that allows us
     * to hide the details in supporting older platforms, and give
     * a useful log message if remote control is not supported.
     * 
     * @param audioManager
     */
    public void register(AudioManager audioManager) {
        if (sHasRemoteControlAPIs) {
            mRegisteredAudioManager = audioManager;
            RemoteControlHelper.registerRemoteControlClient(audioManager, this);
        } else {
            Log.i(TAG, "Cannot register remote control client.  This platform does not have a remote control");
        }
    }

    /**
     * Test if remote control is supported.  This is important to check
     * when attempting to access any RemoteControlClient.* constants, to
     * avoid ClassNotFoundExceptions.
     * 
     * @return
     */
    public boolean isSupportsRemoteControl() {
        return sHasRemoteControlAPIs;
    }

    /**
     * Unregister this client.  This uses the same audio manager passed into
     * register, to avoid having to get it again in the consumer code.
     * 
     */
